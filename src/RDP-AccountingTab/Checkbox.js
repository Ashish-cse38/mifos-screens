...
const Checkboxes = () => {
  ...
  useEffect(() => {
    if(localStorage.getItem('RDP-personal-details-error')){
      setPd_color('#FD5E1A');
      setPd_icon(cross_icon);
    }

    if(localStorage.getItem('RDP-currency-error')){
      setC_color('#FD5E1A');
      setC_icon(cross_icon);
    }

    if(localStorage.getItem('RDP-terms-error')){
      setT_color('#FD5E1A');
      setT_icon(cross_icon);
    }


    if(localStorage.getItem('RDP-settings-error')){
      setS_color('#FD5E1A');
      setS_icon(cross_icon);
    }

    if(localStorage.getItem('RDP-IRC-error')){
      setIrc_color('#FD5E1A');
      setIrc_icon(cross_icon);
    }

    if(localStorage.getItem('RDP-charges-error')){
  ...
  return (
    <div 
      className="RDP-Accounting-checkbox-div mx-auto d-flex my-4"
    >
      <div style={{width: '14%'}}>
